...
%%
datarun = load_data('/Volumes/All_Staff/lab/Experiments/Array/Analysis/2018-11-16-0/data002/data002');
%%
...
%%
number_of_spikes = length(datarun.spikes{100})
...
histogram(datarun.spikes{1},'BinWidth',5)
...
nframe = (datarun.triggers(2) - datarun.triggers(1)) .* 60
...
t10 = datarun.triggers(20);
windowall = [];
for i = 1:length(datarun.spikes)
    neuroni = datarun.spikes{i};
    idx = neuroni < t10 + 3 & neuroni > t10;
    windowi = neuroni(idx);
    windowall = [windowall; windowi];
end
histogram(windowall)
figure(gcf)
...
datarun = load_data('/Volumes/All_Staff/lab/Experiments/Array/Analysis/2013-11-27-0/data001/data001');
...
datarun = load_sta(datarun, 'load_sta', 'all', 'verbose', 1);
%%
...
% mostly empty.  That is because the receptive field (RF) and time courses
...
%%
% Let's plot the receptive field mosaics for ON type1 cells
...
% plot_rf_summaries(datarun, 'duplicates', 'plot_fits', true)
...
%%
% Exercises: Add cell ID labels.  Make the labels red.  Plot the fits in blue
...
plot_rf_summaries(datarun, 'OFF type1', 'plot_fits', true, 'label', true, 'label_color', [1,0,0])

radiusOn1 = mean(get_rf_fit_radius(datarun, 'ON type1'))
radiusOff1 = mean(get_rf_fit_radius(datarun, 'OFF type1'))
radiusOn3 = mean(get_rf_fit_radius(datarun, 'ON type3'))
%%
...
% this is broken. slider did not show up.
%%
% you can also do
...
% cannot see any particularly noisy time course. 
%%
...
% upregulated the threshold for significant stixels (marks) to reduce noise
% / false positive signif stixel
