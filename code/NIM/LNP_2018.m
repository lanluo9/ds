...
for mm = 1:size(pred_rates,2)
    ...
    plot(pred_rate_now(bgn:fin), 'Color', color(mm,:), 'LineWidth', 1)
    hold on
end
...
for nn = 1:5
    R2(nn) = 1 - mean( (spk_per_bin' - pred_rates(:,nn)) .^2) / var(spk_per_bin)
end
sum(R2>0) / size(R2,1) / size(R2,2) 

save(['perf-' num2str(cell_id) '.mat'], '-v7.3')
disp(['saved perf for cell' num2str(cell_id)])
