...
%% use matching pre and post in a single 2s trial
...
%     figure('units','normalized','outerposition',[0 0 1 1]) 
    ...
    for test = 1 : ntest
        for flash_intensity = 2 : length(marker) % exclude dark==990. should improve by excluding 1ms here
            ...
            sample_size = min(length(trial_num_post), length(trial_num_pre));
            order_post = trial_num_post; % datasample(trial_num_post, sample_size, 'Replace', false);
            order_pre = trial_num_pre; % datasample(trial_num_pre, sample_size, 'Replace', false);
    ...
    ylabel('probability correct')
    ylim([0.4, 1.05])
    saveas(gcf, [num2str(ds_slave_now(cellnum)) '-2AFC-fit-matchprepost' '.png'])
    disp(['saved fig for ', num2str(ds_slave_now(cellnum))])
    close
end
disp('done')
