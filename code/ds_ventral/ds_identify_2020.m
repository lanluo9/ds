...
ds_cells = ds_cells'; % turn to comment later!

% for i = 1:length(normal_latency)
%     single_ds_id = normal_latency(i,1); 
%     single_ds_index = ds_cells(ds_cells(:,2)==single_ds_id, 1);
%     if isempty(single_ds_index)
%         disp([num2str(single_ds_id), ' not found in datarun.cell_id']) % 3257, 3588
%     end
% end
...
for i = 1:3
    tmp = ismember(datarun.cell_ids, ds_master_all);
    selected_indices{1} = find(tmp, length(tmp))'
    tmp = ismember(datarun.cell_ids, ds_master_normal);
    selected_indices{2} = find(tmp, length(tmp))'
    tmp = ismember(datarun.cell_ids, ds_master_latency);
    selected_indices{3} = find(tmp, length(tmp))'
end
...
dsi_all = [mean(vector_mags_120(selected_indices{1})), mean(vector_mags_240(selected_indices{1})), mean(vector_mags_480(selected_indices{1}))];
dsi_normal = [mean(vector_mags_120(selected_indices{2})), mean(vector_mags_240(selected_indices{2})), mean(vector_mags_480(selected_indices{2}))];
dsi_latency = [mean(vector_mags_120(selected_indices{3})), mean(vector_mags_240(selected_indices{3})), mean(vector_mags_480(selected_indices{3}))];

error_all = [std(vector_mags_120(selected_indices{1})), std(vector_mags_240(selected_indices{1})), std(vector_mags_480(selected_indices{1}))];
error_normal = [std(vector_mags_120(selected_indices{2})), std(vector_mags_240(selected_indices{2})), std(vector_mags_480(selected_indices{2}))];
error_latency = [std(vector_mags_120(selected_indices{3})), std(vector_mags_240(selected_indices{3})), std(vector_mags_480(selected_indices{3}))];

h = errorbar(tp_axis, dsi_all, error_all, 'b', 'LineWidth', 3)
% alpha(h, 0.2)
% h.FaceAlpha = 0.2;
hold on
errorbar(tp_axis, dsi_normal, error_normal, 'g', 'LineWidth', 2)
hold on
errorbar(tp_axis, dsi_latency, error_latency, 'r--', 'LineWidth', 1)
xticks(tp_axis)
xticklabels({'120','240','480'})
xlabel('TP')
ylabel('DSI magnitude')
...
%% histogram of DSI magnitude

bins = 20;
figure
subplot(3,3,1)
hist(vector_mags_120(selected_indices{1}), bins)
xlim([0.7 1.6])
subplot(3,3,2)
hist(vector_mags_120(selected_indices{2}), bins)
xlim([0.7 1.6])
subplot(3,3,3)
hist(vector_mags_120(selected_indices{3}), bins)
xlim([0.7 1.6])

% figure
subplot(3,3,4)
hist(vector_mags_240(selected_indices{1}), bins)
xlim([0.8 1.6])
subplot(3,3,5)
hist(vector_mags_240(selected_indices{2}), bins)
xlim([0.8 1.6])
subplot(3,3,6)
hist(vector_mags_240(selected_indices{3}), bins)
xlim([0.8 1.6])

% figure
subplot(3,3,7)
hist(vector_mags_480(selected_indices{1}), bins)
xlim([0.8 2.3])
subplot(3,3,8)
hist(vector_mags_480(selected_indices{2}), bins)
xlim([0.8 2.3])
subplot(3,3,9)
...
ds_master_all = [ds_master_normal; ds_master_latency];
tmp = ismember(datarun.cell_ids, ds_master_all);
selected_indices = find(tmp, length(tmp))';
