...
for t = 1 : sample_size
    trial_null = [binned(trial_len*(order_null(t)-1)+section_idx(1,1)+1 : trial_len*order_null(t)+section_idx(1,1))];
    other_null = sum_null - trial_null;
    mean_null = other_null ./ (length(trial_num_null) - 1);
    
    trial_flash = [binned(trial_len*(order_null(t)-1)+section_idx(3,1)+1 : trial_len*order_null(t)+section_idx(3,1))];
    other_flash = sum_flash_section - trial_flash;
...
% range_null = section_idx(section_idx(:,end)==990, 1:2)
% range_flash = [];
