...
for i = 1 : length(ds_cell_ids)
    ...
    title(['data0', num2str(dataset_num), '. TP = ', num2str(grat_TPs(tp_set)), '. id = ', num2str(single_ds_id), '. TP = ', num2str(tp_set)])
    
    saveas(gcf, ['repro0304-', num2str(grat_TPs(tp_set)),'-', num2str(single_ds_id), '.png'])
    close
end
