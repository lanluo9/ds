...
%         binnum = ceil(spike_counts(rgc, g_dir) / datarun.stimulus.repetitions / 2 + 0.001); % number of bins = half of average spike number per rep. if uniform dist, binned=[2,2,2...2]
        ...
        binned_mean{rgc, g_dir} = binned_mean{rgc, g_dir} / (norm(binned_mean{rgc, g_dir}) + 0.001) / datarun.stimulus.repetitions;
    end
end
...
% similarity_index = zeros(num_rgcs, length(datarun.stimulus.params.DIRECTION), datarun.stimulus.repetitions);

for rgc = 1:num_rgcs
    for g_dir = 1:length(datarun.stimulus.params.DIRECTION)
        for g_rep = 1:datarun.stimulus.repetitions
            binned_match(rgc, g_dir, g_rep) = binned{rgc, g_dir, g_rep} / (norm(binned{rgc, g_dir, g_rep}) + 0.001) * binned_mean{rgc, g_dir}';
        end
    end
end
