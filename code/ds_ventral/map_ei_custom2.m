function [cell_list_map, failed_cells, output_matrix] = map_ei_custom2(master_datarun, slave_datarun, varargin)
% 
% map_ei
%
...
%sort
failed_cell_count = 0;
cell_list_map = cell(3,length(master_datarun.cell_ids));
for i = 1:length(master_indices)
    ...
    if largest_corr >= corr_threshold 
        ...
        cell_list_map{1,master_indices(i)}=[];
        cell_list_map{2,master_indices(i)}=[];
        cell_list_map{3,master_indices(i)} = [];
    end
end
