...
%         subplot(length(marker), 1, m)  
        ...
        for s = 1:length(section_id_seq)
            ...
            prestim = 2; % also plot 1 sec of firing rate before stim
            len_whole = 2 + prestim; % either way, whole length of firing rate = 3s
            nbin = len_whole / len_bin;

            peristim_binned = zeros(rep_max, nbin);
            for rep = 1 : rep_step : rep_max
                ...
                [peristim_binned(rep, :), ~] = histcounts(spike_time_peristim, nbin);
            end
            
            firing_rate = movmean( sum(peristim_binned, 1), len_window)
