...
%% visualize count for wrong vs correct trials

disp('visualize count for wrong vs correct trials')
binsize = 0.020;
trial_len = 2 / binsize; % pretend all trial length = 2s
binnum = datarun.duration / binsize;
edges = linspace(0, datarun.duration, binnum);
...
%     ntest = 10;
    ...
    counter_all = cell(length(marker),1);
...
%     for test = 1 : ntest
        for flash_intensity = length(marker) :-1: 2 % exclude dark==990. should improve by excluding 1ms here
            ...
            corrpos = zeros(sample_size, 1);
            counter_fid = zeros(sample_size, 2);
            for t = 1 : sample_size
                if scale == 1
                    if order_pre(t) <= ntrial(fid_seq(1))
                        trial_flash_pre = binned(trial_len*(order_pre(t)-1)+section_idx(fid_seq(1),1)+1 : ...
                ...
                counter_fid(t,1) = counter_pre;
                counter_fid(t,2) = counter_post;
            end
...
for cellnum = 1 : length(ds_slave_now)
    ...
    for sub = 1 : 3 * length(intensity_seq)
        ...
        remark = mod(sub, 3); if remark == 0, remark = 3; end
        
        pre_inten = pre{intensity,1};
        pre_inten_remark = pre_inten(mark{intensity,remark});
        post_inten = post{intensity,1};
        post_inten_remark = post_inten(mark{intensity,remark});
        
        histogram(pre_inten_remark, 10, 'FaceColor','r', 'EdgeColor','w')
        hold on
    ...
    disp(['fig for ', num2str(ds_slave_now(cellnum))])
    close
end    
